-- Add the per-user/per-day rate-limit ledger column + branch for the new
-- `generate-words` AI action (topic-based vocabulary generation).
-- Mirrors the existing per-action columns added in 20260602000000.

ALTER TABLE public.api_usage
  ADD COLUMN IF NOT EXISTS generate_words_count integer NOT NULL DEFAULT 0;

-- Re-declare incr_api_usage with `generate-words` in the allowlist + its branch.
...
BEGIN
  IF p_action NOT IN ('chat', 'evaluate', 'transcribe', 'turn', 'enrich', 'generate-scenario', 'generate-words') THEN
  ...
  ELSIF p_action = 'generate-words' THEN
    INSERT INTO public.api_usage (user_id, usage_date, generate_words_count)
    VALUES (p_user_id, today, 1)
    ON CONFLICT (user_id, usage_date) DO UPDATE
      SET generate_words_count = api_usage.generate_words_count + 1,
          updated_at = now()
    RETURNING generate_words_count INTO new_count;
  END IF;
